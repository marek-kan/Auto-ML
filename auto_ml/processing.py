...
def plot_feature_imp(weights, columns, categories=None, to_plot=10, regression=False):
    ...
    if regression:
        ...
        plt.bar(range(to_plot), plot_df['weight'])
        plt.xticks(range(to_plot), plot_df['feature'], rotation=40, ha='right')
        ...
        if weights.shape[1]==1:
            ...
            for i, category in enumerate(categories):
                ...
                axs[i].bar(range(to_plot), plot_df[f'weight_{category}'])
                axs[i].set_xticks(range(to_plot))
                axs[i].set_xticklabels(plot_df['feature'], rotation=40, ha='right')
